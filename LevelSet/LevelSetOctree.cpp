...
namespace WR
{
    ...
    bool LevelsetOctree::construct(Polyhedron_3& geom, size_t maxLvl)
    {
        release();
        nMaxLevel = maxLvl;
    ...
    void LevelsetOctree::constructChildren(Node* root)
    {
    ...
    void LevelsetOctree::release()
    {
        releaseExceptDt();
        dt.clear();
    }

    void LevelsetOctree::releaseExceptDt()
    {
    ...
    void LevelsetOctree::computeGradient()
    {
        Vector_3 step[3];
    ...
    void LevelsetOctree::computeMinDistance(Node* node)
    {
        static int count = 0;
        for (size_t i = 0; i < 8; i++)
        {
    ...
    int LevelsetOctree::determineSign(int type, const Point_3& p, const Vector_3& diff, size_t triIdx) const
    {
        switch (type)
        {
        case 0:
    ...
    int LevelsetOctree::detSignOnFace(const Point_3& p, const Vector_3& diff, size_t triIdx) const
    {
        if (diff * triList[triIdx].normal < 0.0) return 1;
        else return -1;
    }

    int LevelsetOctree::detSignOnEdge(const Point_3& p, const Vector_3& diff, size_t triIdx, int seq) const
    {
    ...
    int LevelsetOctree::detSignOnVertex(const Point_3& p, const Vector_3& diff, size_t triIdx, int seq) const
    {
        auto edge = triList[triIdx].fh->facet_begin();
        for (size_t i = 0; i < seq; i++, edge++);
    ...
    double LevelsetOctree::minSquaredDist(const Point_3& p, Iterator begin, Iterator end, Vector_3* diff, size_t* pTriIdx, int* pType) const
    {
        assert(begin != end);
    ...
    double LevelsetOctree::minDist(const Cube& bbox, const Point_3& p)
    {
        double dist = 1.e9;
...
#define MIN_TEST(d, d0)\
    ...
    LevelsetOctree::Node* LevelsetOctree::createRootNode(const Polyhedron_3& geom)
    {
    ...
    void LevelsetOctree::computeTripleFromBbox(Cube& triple, const Cube& bbox) const
    {
    ...
    LevelsetOctree::Node* LevelsetOctree::createNode()
    {
        Node* node = new Node;
    ...
    float LevelsetOctree::queryExactDistance(const Point_3& p) const
    {
        return sqrt(minSquaredDist(p, pRoot->eList.begin(), pRoot->eList.end()));
    }

    float LevelsetOctree::queryDistance(const Point_3& p) const
    {
    ...
    bool LevelsetOctree::queryGradient(const Point_3& p, Vector_3& grad) const
    {
