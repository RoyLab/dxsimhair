...
namespace xhair
{
	namespace
	{
		...
		void solveSingleGroupDeprecated(std::vector<Point_3>& points, WR::VecX& x, float dr)
		{
			...
			{
				std::list<IntWrapper<Point_3>> tmps;
	...
	void GroupPBD::computeVecb(int gId, Point3* pts, float dr, WR::VecX& b)
	{
	...
	void GroupPBD::solveSingleGroup(int gId, const Tree* pTree, Point3* p0, WR::VecX& x, int pps, bool pass0)
	{
	...
	void GroupPBD::solveSampled(HairGeometry* hair)
	{
		...
		// export points
		for (size_t i = 0; i < hair->nParticle; i++)
			hair->position[i] = Point3(x[3 * i], x[3 * i + 1], x[3 * i + 2]);
	}
	...
	public:
		TbbPbdItem(GroupPBD* pp, Tree* tr, Point3* p0, Point3* p1, float d, HairGeometry* hair, int pass):
			tree(tr), iPass(pass)
		{
			pos0 = p0;
			newpos = p1;
		...
		void operator()(const blocked_range<size_t>& r) const {
			...
			for (size_t i = r.begin(); i != end; ++i)
			{
				...
				for (size_t i = 0; i < arr.size(); i++)
					newpos[arr[i]] = Point3(x[3*i], x[3*i+1], x[3*i+2]);
			}
		}
	};
	...
	void GroupPBD::solveFull(HairGeometry* hair)
	{
		...
		//f.write((char*)hair->position, sizeof(Point3)*hair->nParticle);
		//f.close();
		return;
		// dump code end
		...
		Point3* allocMem = new Point3[hair->nParticle];
		Point3 *p0 = hair->position.get(), *p1 = allocMem;
		...
		{
			float t = 0.0f;
			std::memcpy(p1, p0, sizeof(Point3) * hair->nParticle);
			for (int j = 0; j < nHairParticleGroup; j++)
			{
				...
				for (size_t k = 0; k < gIds.size(); k++)
					p1[gIds[k]] = Point3(x[3*k], x[3*k+1], x[3*k+2]);
			}
        ...
        if (hair->position.get() != p1)
        {
            hair->position.reset(allocMem);
        }
        else
        {
